...
from calculate.services.wallet_score import calculate_credit_score
    ...
    def _export_batch(self, wallets_batch_indicates):
        for batch_idx in wallets_batch_indicates:
            try:
                ...
                wallets = self._graph.get_wallets_with_batch_idx(
                    batch_idx=batch_idx,
                    ...
                    try:
                        ...
                        for key, timestamp in wallet.get("scoringTimestamps").items():
                            credit_score, elements = calculate_credit_score(
                                ...
                                current_time=timestamp, return_elements=True
                            )
                            updated[key] = update_scores_properties(wallet, credit_score, elements, current_time,
                                                                    multichain=self.multichain)
