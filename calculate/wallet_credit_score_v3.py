...
from calculate.services.wallet_score_v3 import calculate_credit_score, calculate_credit_score_with_info_return
...
from job.scoring_job_v3 import WalletScoresJob
from utils.csv_utils import write_csv

logger = get_logger('Wallet Credit Score V3')


class WalletsCreditScore:
    def __init__(self, graph, statistic_path='data', k=30, tokens=None):
        self._graph = MongoDB(graph)
        self._k = k
    ...
    def calculate_wallet_with_info(self, wallet, h=10, save=True, is_backup=False):
        ...
        results = {
            ...
            'scores': {
                ...
                'frequency_of_dapp_interaction': x2[0],
                'number_of_interacted_dapps': x2[1],
                # 'types_of_interacted_dapps': x2[2],
                # 'reputation_of_interacted_dapps': x2[3],
                'transaction_amount': x3[1],
                'frequency_of_transaction': x3[2],
                'age_of_accounts': x4[0],
                'number_of_liquidations': x4[3],
                'total_value_of_liquidations': x4[4],
                'loan-to-balance_ratio': x5[0],
                'loan-to-investment_ratio': x5[1],
                'investment-to-total-asset_ratio': x6[0],
                # 'return_on_investment': x6[1],
                'token_score': x7[0],
                'nft_score': x7[1]
            },
            'detail': info
